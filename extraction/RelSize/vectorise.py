...
class UnlabeledImageFolder(Dataset):
    def __init__(self, transform):
        ...
        fls = open(ROOT + FILES, 'r')
        self.images = [f.split()[0] for f in fls.readlines()]
        fls.close()

    def __getitem__(self, index):
        image = Image.open(ROOT + 'input/' + self.images[index]).convert('RGB')
...
def my_app(cfg: DictConfig) -> None:
    ...
    with open('pca_eigen.obj', 'rb') as input_file:
    ...
    for i, (img, name) in enumerate(loader):
        with torch.no_grad():
            ...
            for j in range(img.shape[0]):
                ...
                res = arr[j]
                np.savez_compressed(ROOT + 'R/' + new_name, a=res)
